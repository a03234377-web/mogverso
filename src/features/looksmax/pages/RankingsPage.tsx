...
import { AdBanner } from "@/features/looksmax/components/AdBanner";
import { Icon, IconLabel } from "@/components/icons";
import { EntryVoteCard } from "@/features/looksmax/components/EntryVoteCard";
import { CountdownDigits } from "@/features/looksmax/components/ui/CountdownDigits";
import { HeroBadge, HeroSection } from "@/features/looksmax/components/ui/HeroSection";
import { MoversCard } from "@/features/looksmax/components/ui/MoversCard";
import { RankRow } from "@/features/looksmax/components/ui/RankRow";
import { SectionTitle } from "@/features/looksmax/components/ui/SectionTitle";
import { useCountdown } from "@/features/looksmax/hooks/useCountdown";
import { useLooksMaxNavigate } from "@/features/looksmax/shell/LooksMaxShell";
import type { Mover, RankedEntry } from "@/features/looksmax/lib/ranking";
    ...
    <div id="page-rankings" className="block animate-fade-up">
      ...
      <div className="mx-auto mt-7 flex flex-wrap items-start justify-center gap-5 px-5 max-md:mt-3 max-md:flex-col max-md:gap-3 max-md:px-4">
        <div className="flex w-[220px] flex-col items-center gap-2.5 max-md:w-full max-md:flex-row max-md:gap-3.5">
          <button
            type="button"
            className="lm-focus-ring-on-gold w-full min-h-[58px] cursor-pointer rounded-[14px] border-none bg-[linear-gradient(135deg,var(--color-lm-green2),var(--color-lm-gold))] px-8 py-3 font-display text-[1.3rem] tracking-[2px] text-black shadow-[0_0_24px_rgba(46,204,113,0.35)] transition-all duration-250 hover:scale-105 max-md:min-h-[50px] max-md:flex-1 max-md:rounded-xl max-md:px-4 max-md:py-2.5 max-md:text-[1.05rem]"
        ...
        <div className="flex w-[220px] flex-col items-center gap-2.5 max-md:w-full max-md:flex-row max-md:gap-3.5">
          <button
            type="button"
            className="lm-focus-ring-on-gold w-full min-h-[58px] cursor-pointer rounded-[14px] border-none bg-[linear-gradient(135deg,#ff6b35,var(--color-lm-gold))] px-8 py-3 font-display text-[1.3rem] tracking-[2px] text-black shadow-[0_0_24px_rgba(255,107,53,0.35)] transition-all duration-250 hover:scale-105 max-md:min-h-[50px] max-md:flex-1 max-md:rounded-xl max-md:px-4 max-md:py-2.5 max-md:text-[1.05rem]"
      ...
      <div
        ...
        id="moversGrid"
      >
        <MoversCard title="Mayores Subidas" titleIcon="trending-up" variant="up" movers={upMovers} />
        <MoversCard title="Últimos Movimientos" titleIcon="trending-down" variant="down" movers={downMovers} />
      </div>
      ...
      <div className="mx-auto max-w-[1100px] px-5 pb-12 max-md:px-2.5 max-md:pb-20">
        ...
        <div className="flex flex-col gap-2" id="rankList">
          {entries.map((entry, i) => (
            <RankRow key={entry.name} entry={entry} index={i} onOpenProfile={openProfile} />
          ))}
        </div>
      </div>
