...
import type { NavigationProps, PageId } from "@/features/looksmax/types";
import { LOOKSMAX_PATHS } from "@/features/looksmax/routes";
...
type NavTabId = Exclude<PageId, "profile">;
...
const BNAV: { id: NavTabId; label: string; icon: IconName; badge?: string; tabIdx: number }[] = [
      ...
      >
        {DESKTOP_TABS.map((tab, idx) => (
          <Link
            ...
            className={cn(
              "lm-focus-ring flex h-[var(--lm-nav-height)] shrink-0 cursor-pointer items-center gap-1.5 whitespace-nowrap border-0 bg-transparent px-3 font-sans text-[0.7rem] font-bold uppercase tracking-[0.8px] text-lm-text2 no-underline transition-all duration-250 select-none hover:text-lm-text",
        ...
        >
          {BNAV.map((tab, idx) => (
            <Link
              ...
              tabIndex={getTabIndex(idx)}
              className={cn(
                "lm-focus-ring relative flex h-full flex-1 flex-col items-center justify-center gap-1 border-0 border-t-2 border-transparent bg-transparent px-0.5 py-1 font-sans no-underline transition-all duration-200",
...
function MoreLinkItem({
  ...
  return (
    <Link
      href={href}
      role="menuitem"
      className="lm-focus-ring flex w-full cursor-pointer items-center gap-3 rounded-lg border-0 bg-transparent px-1.5 py-2.5 text-left no-underline hover:bg-lm-bg3"
      onClick={onClose}
    >
      <Icon name={icon} size={20} className="text-lm-gold" />
      <div>
        <div className="text-[0.85rem] font-extrabold text-lm-text">{title}</div>
