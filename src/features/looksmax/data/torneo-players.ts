import { creatorImage } from "@/features/looksmax/data/creator-images";
...
import type {
  ...
  TorneoPlayer,
  TorneoState,
} from "@/features/looksmax/lib/types";
...
const OCTAVOS_IDS = ["oct_0", "oct_1", "oct_2", "oct_3", "oct_4", "oct_5", "oct_6", "oct_7"] as const;
...
export function buildCuartosMatches(winners: (string | null)[]): Record<string, TorneoMatch> {
...
export function buildSemisMatches(winners: (string | null)[]): Record<string, TorneoMatch> {
...
async function doAdvanceTorneoPhase(
  ...
  if (state.phase === PHASES.CUARTOS_VOTING) {
    ...
    const ok = await fb.atomicAdvanceTorneoPhase(PHASES.CUARTOS_VOTING, newState as Record<string, unknown>);
    if (!ok) {
  ...
  if (state.phase === PHASES.SEMIFINALS_PROMO) {
    ...
    const ok = await fb.atomicAdvanceTorneoPhase(PHASES.SEMIFINALS_PROMO, newState as Record<string, unknown>);
    if (!ok) {
      const fresh = await fb.getTorneoState();
      return (fresh as TorneoState | null) ?? state;
    }
  ...
  if (state.phase === PHASES.SEMIFINALS_VOTING) {
    ...
    const ok = await fb.atomicAdvanceTorneoPhase(PHASES.SEMIFINALS_VOTING, newState as Record<string, unknown>);
    if (!ok) {
      const fresh = await fb.getTorneoState();
      return (fresh as TorneoState | null) ?? state;
    }
  ...
  if (state.phase === PHASES.BREAK_FINAL) {
    ...
    const ok = await fb.atomicAdvanceTorneoPhase(PHASES.BREAK_FINAL, newState as Record<string, unknown>);
    if (!ok) {
      const fresh = await fb.getTorneoState();
      return (fresh as TorneoState | null) ?? state;
    }
  ...
  if (state.phase === PHASES.FINAL_VOTING) {
    ...
    const ok = await fb.atomicAdvanceTorneoPhase(PHASES.FINAL_VOTING, newState as Record<string, unknown>);
    if (!ok) {
      const fresh = await fb.getTorneoState();
      return (fresh as TorneoState | null) ?? state;
    }
