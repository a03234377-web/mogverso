...
import type { RankOverrides, RankVoteMyVote, RankVoteRound } from "@/features/looksmax/lib/types";
    ...
    async (round: RankVoteRound | null) => {
      ...
      if (round.endTime <= now) {
        if (!resolvingRef.current) {
          resolvingRef.current = true;
          if (arenaActive) setTransitioning(true);
          await fb.resolveRVIfNeeded(round as Record<string, unknown>).catch(console.error);
          resolvingRef.current = false;
        }
        return;
      }
