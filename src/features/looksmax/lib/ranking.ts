import { RANKERS, type Ranker } from "@/features/looksmax/data/rankers";
import type { RankMovement, RankMovements, RankOverrides } from "@/features/looksmax/lib/types";
...
export function buildRankedList(
  ...
  return rankedNames.flatMap((name, i) => {
    ...
    const mov = movements?.[name];
    const movement =
      mov && mov.ts > cutoff && mov.delta > 0 ? mov : null;
    return [
      {
        name,
        rank: i + 1,
