...
import { startTorneoOctavosFromSeed } from "@/lib/firebase/server-torneo";
...
export async function POST(request: Request) {
  ...
  try {
    const state = await startTorneoOctavosFromSeed();
    return jsonOk({ state });
  } catch (err) {
    console.error("[api/admin/torneo/start-now]", err);
    return jsonError("start_failed", 500);
  }
}
